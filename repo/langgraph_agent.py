...
def retrieve_node(state: AgentState):
    import time; time.sleep(4) # Pacing API calls
    ...
    Output only the option name in strict JSON format: {{"datasource": "..."}}
    Question: {question}
    """
    
    msg = [
        SystemMessage(content="You are a routing agent for financial questions. Output ONLY valid JSON."),
    ...
    try:
        ...
        # Remove markdown formatting if the LLM adds it (e.g., ```json ... ```)
        ...
        target = json_output.get("datasource", "none").lower()
        if target not in ["apple", "tesla", "both", "none"]:
            target = "none"
        print(f"   Routed to: {target}")
    except Exception as e:
        print(colored(f"⚠️ Error parsing JSON from router: {e}. Falling back to 'both'.", "yellow"))
        target = "both"
...
def generate_node(state: AgentState):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an expert financial analyst. Use the provided context to answer the question. \n"
                    ...
                    "2. Year Precision: When reading financial tables, strictly distinguish between the 2024, 2023, and 2022 columns.\n"
                    "3. If the context doesn't contain the answer (even after retries), you must honestly state 'I don't know' instead of hallucinating.\n"
                    "4. You MUST strictly cite sources for your claims using the exact format: [Source: Apple 10-K] or [Source: Tesla 10-K].\n\nContext:\n{context}"),
        ("human", "{question}"),
    ])
...
def rewrite_node(state: AgentState): 
    ...
    msg = [ 
        HumanMessage(content=f"The previous search for the question '{question}' yielded irrelevant results. \n"
                             f"Please rewrite this question to be more specific and use precise financial terminology (e.g., transform 'how much did they spend on new tech' into 'Research and Development expenses'). \n"
                             f"Output ONLY the new question text without any quotes or explanations.")
    ]
    response = llm.invoke(msg)
...
def run_legacy_agent(question: str):
    ...
    template = """You are an Autonomous Multi-Doc Financial Analyst.
    Answer the following questions as best you can. You have access to the following tools:

    {tools}

    CRITICAL INSTRUCTIONS:
    1. English Only: Your Final Answer MUST be in English, regardless of the language of the input question.
    2. Year Precision: When reading financial tables, strictly distinguish between the 2024, 2023, and 2022 columns.
    3. Honesty: If the exact 2024 figure or requested info is not found, you MUST state "I don't know" instead of guessing.
