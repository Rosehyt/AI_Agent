...
def build_vector_dbs():
    ...
    for key, filename in FILES.items():
        ...
        # 2. Embed & Store (With API Rate Limit Handling)
        print("   - Embedding and storing with Rate Limit Handling... (This may take a while)")
        
        import time
        vectorstore = Chroma(persist_directory=persist_dir, embedding_function=embeddings)
        
        # 將資料切成較小的批次(每批 20 個 chunks)，並在批次之間休息以規避 API Token 限制
        batch_size = 20
        for i in range(0, len(splits), batch_size):
            batch = splits[i:i+batch_size]
            vectorstore.add_documents(batch)
            print(f"     ✅ Processed {min(i+batch_size, len(splits))}/{len(splits)} chunks. Sleeping...")
            time.sleep(5) # 強制休息 5 秒
